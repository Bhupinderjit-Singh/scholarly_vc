...
### `web/lib/db`
- `client.ts`: `pg.Pool({ connectionString, max: 5, ssl })` wrapped by `drizzle(pool, { schema })`. The `pg` driver works identically against Neon's pooled connection string in production and a local or CI Postgres in tests, which is why it is preferred over the Neon HTTP driver.
